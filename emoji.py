...
recipient_dict = {
    ...
    "archana": "archanasapkal20@gmail.com"
}
def speak(str):
    encoded_str = str.encode('utf-8').decode(sys.stdout.encoding)
    print(encoded_str)
    engine.say(encoded_str)
    engine.runAndWait()
...
def sendmail(recipient_email):
    ...
    str = "Do you want to add an emoji to the email body? Say 'confirm' or 'no'.\n"
    speak(str)
    response_emoji = listen().lower()

    if response_emoji == 'confirm':
        str = "Please speak the type of emoji you want to add (e.g., 'sad', 'happy', 'angry','Heart','Congratulations','Computer').\n"
        speak(str)
    ...
    str = "Do you want to add any recipients in CC? If yes, please say 'yes', if no then say 'cancel'.\n"
    speak(str)
    add_cc_recipients = listen().lower()  # Convert to lowercase

    if add_cc_recipients == 'yes':
        ...
        if cc_recipients:
            ...
            str = "No recipients added to CC. Sending email without CC.\n"
            speak(str)

    elif add_cc_recipients == 'cancel':
        ...
        try:
            server.login(unm, pwd)
            server.sendmail(unm, [recipient_email], msg.as_string())
            server.quit()
        ...
        str = "Invalid response. Sending email without CC.\n"
        speak(str)
...
def read_latest_unread_email(unm, pwd):
    ...
    if response_delete == 'delete':
        ...
        reply_message["From"] = "komalmore1912@gmail.com"  # Replace with your email
